...
import { RemediationRule, type RemediationRuleDocument } from "../../../../../models/RemediationRule.js";
...
export class MongoRemediationRuleRepository implements IRemediationRuleRepository {
  ...
  async update(ruleId: string, updates: UpdateRemediationRuleInput): Promise<RemediationRuleDTO | null> {
    ...
    if (updates.match) {
      if (updates.match.pipelineId !== undefined) setFields["match.pipelineId"] = updates.match.pipelineId;
      if (updates.match.stageName !== undefined) setFields["match.stageName"] = updates.match.stageName;
      if (updates.match.anyPatterns !== undefined) setFields["match.anyPatterns"] = updates.match.anyPatterns;
      if (updates.match.anyHintSubstrings !== undefined) setFields["match.anyHintSubstrings"] = updates.match.anyHintSubstrings;
    }
    if (updates.action) {
      if (updates.action.type !== undefined) setFields["action.type"] = updates.action.type;
      if (updates.action.maxAttempts !== undefined) setFields["action.maxAttempts"] = updates.action.maxAttempts;
      if (updates.action.backoffSeconds !== undefined) setFields["action.backoffSeconds"] = updates.action.backoffSeconds;
    }
    if (updates.auto) {
      if (updates.auto.enabled !== undefined) setFields["auto.enabled"] = updates.auto.enabled;
      if (updates.auto.minAttempts !== undefined) setFields["auto.minAttempts"] = updates.auto.minAttempts;
      if (updates.auto.disableBelowSuccessRate !== undefined) setFields["auto.disableBelowSuccessRate"] = updates.auto.disableBelowSuccessRate;
    }
    if (updates.stats) {
      if (updates.stats.attempts !== undefined) setFields["stats.attempts"] = updates.stats.attempts;
      if (updates.stats.saves !== undefined) setFields["stats.saves"] = updates.stats.saves;
      if (updates.stats.failures !== undefined) setFields["stats.failures"] = updates.stats.failures;
      if (updates.stats.lastAppliedAt !== undefined) setFields["stats.lastAppliedAt"] = updates.stats.lastAppliedAt;
      if (updates.stats.lastOutcomeAt !== undefined) setFields["stats.lastOutcomeAt"] = updates.stats.lastOutcomeAt;
    }
