...
import type { IStageRepository, StageDTO, UpdateStageStatusInput } from "../../../../../domain/index.js";
...
export class SqliteStageRepository implements IStageRepository {
  ...
  async findByRunId(runId: string): Promise<StageDTO[]> {
    const rows = this.db.prepare("SELECT * FROM stages WHERE run_id = ? ORDER BY rowid ASC").all(runId) as SqliteStageRow[];
    return rows.map((row) => SqliteStageMapper.toDTO(row));
  }

  async findStage(runId: string, stageName: string): Promise<StageDTO | null> {
    const row = this.db.prepare("SELECT * FROM stages WHERE run_id = ? AND name = ?").get(runId, stageName) as SqliteStageRow | undefined;
  ...
  async updateStatus(
    ...
    const updatedDTO: StageDTO = {
      ...
      metrics: input.metrics
        ? {
            cpuSeconds: input.metrics.cpuSeconds !== undefined ? (input.metrics.cpuSeconds ?? null) : existing.metrics.cpuSeconds,
            cpuPercentAvg: input.metrics.cpuPercentAvg !== undefined ? (input.metrics.cpuPercentAvg ?? null) : existing.metrics.cpuPercentAvg,
            cpuPercentMax: input.metrics.cpuPercentMax !== undefined ? (input.metrics.cpuPercentMax ?? null) : existing.metrics.cpuPercentMax,
            memBytesMax: input.metrics.memBytesMax !== undefined ? (input.metrics.memBytesMax ?? null) : existing.metrics.memBytesMax,
            costUsdEstimated: input.metrics.costUsdEstimated !== undefined ? (input.metrics.costUsdEstimated ?? null) : existing.metrics.costUsdEstimated,
          }
        : existing.metrics,
    };
